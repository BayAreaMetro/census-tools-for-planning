#Tenure Bay Area check
#Note later discovered Carolina is by households
...
#collect total units
ACS_variables <- c(Tot_Units= "B25127_001",
                   Own_Units= "B25127_002",
                   Rent_Units= "B25127_045",
                   Tot_HHs= "B11015_001"
                   )
...
ACS_county_2016 <- get_acs(geography = "county", variables = ACS_variables,
                          state = statenumber, county=baycounties,
                          year=2016,
                          output="wide",
                          survey = ACS_product )

bay_total_units2018 = sum(ACS_county_raw$Tot_UnitsE)
bay_owned_units2018 = sum(ACS_county_raw$Own_UnitsE)
bay_rental_units2018 = sum(ACS_county_raw$Rent_UnitsE)
bay_households2018 = sum(ACS_county_raw$Tot_HHsE)

bay_total_units2016 = sum(ACS_county_2016$Tot_UnitsE)
bay_owned_units2016 = sum(ACS_county_2016$Own_UnitsE)
bay_rental_units2016 = sum(ACS_county_2016$Rent_UnitsE)

#UDP share rent control 
